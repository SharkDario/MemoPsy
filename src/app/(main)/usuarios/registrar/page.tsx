              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="isPsicologo" className="text-gray-200 flex items-center">
                    <Stethoscope className="w-5 h-5 mr-2 text-teal-400" /> Registrar como Psicólogo
                  </Label>
                </div>
...
import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from "@/components/ui/card";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Menu, X, Users, Calendar, FileText, LogOut, Settings, Shield, UserCircle, Briefcase, Activity, Stethoscope, UserPlus as UserPlusIcon } from 'lucide-react';
import { Switch } from "@/components/ui/switch";
...
export default function RegistrarUsuarioPage() {
    ...
    if (isPaciente && hasPermission) {
      ...
      fetch('/api/obras-sociales')
        ...
        .then(data => {
          ...
          const obrasSociales = data.map((item: any) => 
            ObraSocial.fromEntity({
              id: item.id_obra_social?.toString() || item.id?.toString(),
              nombre: item.nombre,
              activo: item.activo !== undefined ? item.activo : true
            })
          );
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const usuario = new Usuario({
        email: formData.email.trim(),
        password: formData.password, // Se hasheará en el backend
        activo: true,
        ultimoAcceso: new Date(),
      });
      ...
      // Agregar datos específicos de paciente si aplica
      if (isPaciente) {
        const obraSocialSeleccionada = obrasSocialesList.find(
          os => os.id === selectedObraSocialId
        );
        
        if (!obraSocialSeleccionada) {
          throw new Error('Obra social seleccionada no válida');
        }
      ...
      alert('Usuario registrado exitosamente con los roles seleccionados.');
      resetForm();
      router.push('/usuarios');
  ...
  const resetForm = () => {
    setFormData({
      ...
      dni: '', fecha_nacimiento: '', numeroLicencia: '', especialidad: '',
    });
    setIsPsicologo(false);
    setIsPaciente(false);
    setSelectedObraSocialId('');
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#152A2A' }}>
      ...
      <main className="container mx-auto px-4 py-8">
        <Card className="max-w-2xl mx-auto" style={{ backgroundColor: '#1D3434', borderColor: '#2A4A4A' }}>
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold" style={{ color: '#F1C77A' }}>Registrar Nuevo Usuario</CardTitle>
            <CardDescription className="text-gray-300">
              Complete los campos para crear una nueva cuenta en el sistema.
            </CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                 ...
                 <div>
                  <Label htmlFor="fecha_nacimiento" className="text-gray-200">Fecha de Nacimiento</Label>
                  <Input id="fecha_nacimiento" name="fecha_nacimiento" type="date" value={formData.fecha_nacimiento} onChange={handleChange} required
                         className="bg-gray-700 text-white border-gray-600 focus:ring-teal-500 focus:border-teal-500 mt-1"
                         style={{ colorScheme: 'dark' }}/>
                  {fieldErrors.fecha_nacimiento && <p className="text-red-500 text-xs mt-1">{fieldErrors.fecha_nacimiento}</p>}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {isPsicologo && (
                  <div className="pl-8 space-y-4 transition-all duration-300 ease-in-out">
                    <div>
                      <Label htmlFor="numeroLicencia" className="text-gray-300">Número de Licencia</Label>
                      <Input id="numeroLicencia" name="numeroLicencia" value={formData.numeroLicencia} onChange={handleChange} required={isPsicologo}
                            className="bg-gray-700 text-white border-gray-600 focus:ring-teal-500 focus:border-teal-500 mt-1"/>
                      {fieldErrors.numeroLicencia && <p className="text-red-500 text-xs mt-1">{fieldErrors.numeroLicencia}</p>}
                    </div>
                    <div>
                      <Label htmlFor="especialidad" className="text-gray-300">Especialidad</Label>
                      <Input id="especialidad" name="especialidad" value={formData.especialidad} onChange={handleChange} required={isPsicologo}
                            className="bg-gray-700 text-white border-gray-600 focus:ring-teal-500 focus:border-teal-500 mt-1"/>
                ...
                {isPaciente && (
                  <div className="pl-8 space-y-4 transition-all duration-300 ease-in-out">
                    <div>
                      <Label htmlFor="selectedObraSocialId" className="text-gray-300">Obra Social</Label>
                      <Select name="selectedObraSocialId" value={selectedObraSocialId} onValueChange={handleObraSocialSelectChange} required={isPaciente}>
                        <SelectTrigger className="bg-gray-700 text-white border-gray-600 focus:ring-teal-500 focus:border-teal-500 mt-1">
                        ...
                        <SelectContent style={{ backgroundColor: '#1D3434', borderColor: '#2A4A4A' }}>
                          {isLoadingObrasSociales ? (
                            ...
                            obrasSocialesList.map(obra => (
                              <SelectItem key={obra.id} value={obra.id} className="text-gray-200 hover:!bg-gray-600 focus:!bg-gray-600">
                                {obra.nombre}
                              </SelectItem>
                            ))
                          )}
